...
interface Exercise { id: string; name: string; nameIt: string | null }
interface Session { id: string; date: Date; _count: { sets: number }; programDay: { name: string } | null }
...
interface ProgressPoint { date: string; maxWeight: number; totalVolume: number }

export function ProgressClient({ exercises, recentSessions }: Props) {
      ...
      {selectedExercise && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">{selectedName?.nameIt ?? selectedName?.name}</CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
              ...
              <div className="h-48 flex items-center justify-center text-zinc-500 text-sm">
                Nessun dato ancora per questo esercizio
              </div>
            ) : (
